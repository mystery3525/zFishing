...
function ENT:SetupDataTables() 
	self:NetworkVar( "Entity", 0, "FMod_OW" ) 
	self:NetworkVar( "String", 0, "FMod_OI" ) 
	self:NetworkVar( "Int", 1, "FMod_CT" ) 
end
...
function ENT:GetOverlayText() 
	local ni = ( self:GetFMod_OW():IsPlayer() and self:GetFMod_OW():Nick() or self:GetFMod_OI() )
	return self.PrintName.."\n"..language.GetPhrase( "xdefm.Bucket2" ).."\n"..language.GetPhrase( "xdefm.Owner" )..": "..ni 
end
...
function ENT:Use( act ) 
	if !act:IsPlayer() or !act:Alive() or ( !xdefm_FriendAllow( act, self:GetFMod_OI() ) and !xdefm_NadAllow( act, self ) ) then return end
	...
	if act:KeyDown( IN_SPEED ) then act:PickupObject( self ) return elseif self:GetFMod_CT() > 0 then
		local ite = self.xdefm_Table[ #self.xdefm_Table ]  
		local giv = xdefm_ItemGive( act, ite ) if !giv then return end
		table.remove( self.xdefm_Table, #self.xdefm_Table ) 
		self:EmitSound( "EpicMetal.ImpactSoft" )
		self:SetFMod_CT( self:GetFMod_CT() -1 ) 
		self.xdefm_Cool = CurTime() +0.5 
	end 
end

function ENT:Touch( ent ) 
	if ent:GetClass() != "xdefm_base" or self.xdefm_Cool > CurTime() or self:GetFMod_CT() >= 16 or ent.xdefm_Trashed then return end 
	self.xdefm_Cool = CurTime() +1
	local aa, bb = xdefm_ItemGet( ent ) 
	if !istable( bb ) or !isnumber( bb.Price ) or !bb.Carryable or !ent:IsPlayerHolding() then return end 
	
	local ow = ent:GetFMod_OW()
	...
	self:SetFMod_CT( self:GetFMod_CT() +1 ) 
	table.insert( self.xdefm_Table, ent:GetFMod_DT() )
	self:EmitSound( "SolidMetal.ImpactSoft" ) 
	ent.xdefm_Trashed = true ent:Remove() 
end

function ENT:Think() 
	if SERVER then if IsValid( self.xdefm_Dummy ) then 
		self.xdefm_Dummy:SetAngles( self:LocalToWorldAngles( Angle( 0, CurTime()*10, 0 ) ) ) 
	end
	self:NextThink( CurTime() +0.01 ) return true end 
	local text = self:GetOverlayText() 
	local ply, tag = LocalPlayer(), GetConVar( "xdefmod_tagdist" ):GetInt()
	if !IsValid( ply ) or tag == 0 or ply:GetPos():DistToSqr( self:GetPos() ) > tag^2 then return end 

	local red = self:GetFMod_CT()/16
	local col = Color( 255, 255 -255*red, 255 -255*red )
	if self:BeingLookedAtByLocalPlayer() then 
		halo.Add( { self }, col, 1, 1, 1 )
		if text != "" then 
...
function ENT:Draw() 
	...
	local red = self:GetFMod_CT()/16  
	local col = Color( 255, 255 -255*red, 255 -255*red )  
	local txt = self:GetFMod_CT().." / 16"
	surface.SetFont( "xdefm_Font3" ) 
